...
def dPandHTC(Fluid,P,H,MFLX,HFLX,Dh,A,Ph,Ep,Angle,SH,refpropm,prt=False,corrModel=0):
    ...
    if P >= CritP:
        ...
        if prt: print("RCLSA RCLSA ", H, Hsatliq)
        if H <= Hsatliq:
            State='liquid'
        elif H>=Hsatgas:
            State='gas'
    ...
    if State == 'supercritical':
        ...
        if HFLX>=0:
            if Fluid == 'CO2':
                ...
                # This is for systematic uncertainty studies.
                if corrModel == 1: #'FlexLine':
                    dP,rm = FlexLineCorrelation(Fluid,MFLX)
                elif corrModel == 2: #'Chisholm': this is the modified version
                    dP,rm=ChisholmCorrelation(Fluid,P,VQ,MFLX,Dh,A, refpropm)
                elif corrModel == 3: # 'Friedel': this is physical
                    dP,rm=FriedelCorrelation(Fluid,P,VQ,MFLX,Dh,A, refpropm)
                elif corrModel == 4: # '2x thome': this is not physical! for uncertainties
                    dP = 2*dP
                    #print(State,VQ)
